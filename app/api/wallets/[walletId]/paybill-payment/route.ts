  ...
  try {
    const transaction = await prisma.transaction.create({
      data: {
        ...
        amount: -amount,
        type: 'b2b_paybill',
        description: body.description || 'Paybill payment',
        status: 'pending',
        reference: `PAYBILL-${Date.now()}`,
        phoneNumber: body.paybill_number || null,
        transactionType: 'b2b_paybill',
        accountReference: body.account_reference || body.account_number || null,
    ...
    return Response.json({ success: false, message: 'Failed to initiate paybill payment. Please try again.' }, { status: 500 });
  }
}