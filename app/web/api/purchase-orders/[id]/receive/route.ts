...
export async function POST(
  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      // Persist any uploaded delivery note files as separate records (supports multiple)
    ...
    console.error('Failed to record stock received:', error);
    return NextResponse.json(
      { error: error?.message || 'Failed to record stock received' },
      { status: 500 }
    );
  }
}
