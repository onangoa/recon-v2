...
export async function GET(
  ...
  try {
    ...
    if (!component) {
      return mobileError('Salary component not found', 404);
    }
    return mobileSuccess(component);
  } catch (error) {
    return mobileError('Failed to fetch salary component', 500);
  }
}
...
export async function PUT(
  ...
  const permCheck = await mobileRequireContractorPermission(request, 'salary_components:update');
  if (!permCheck.authorized) return permCheck.error!;
  const contractorId = permCheck.contractorId!;
  try {
    const { id } = await params;
    ...
    return mobileSuccess(component, 'Salary component updated');
  } catch (error) {
    return mobileError('Failed to update salary component', 500);
  }
}
...
export async function DELETE(
  ...
  const permCheck = await mobileRequireContractorPermission(request, 'salary_components:delete');
  if (!permCheck.authorized) return permCheck.error!;
  const contractorId = permCheck.contractorId!;
  try {
    const { id } = await params;
    ...
    return mobileSuccess(null, 'Salary component deleted');
  } catch (error) {
    return mobileError('Failed to delete salary component', 500);
  }
}
