...
export async function GET(request: NextRequest) {
  const permCheck = await mobileRequireContractorPermission(request, 'salary_slips:read');
  ...
  try {
    ...
    return mobileSuccess(slips);
  } catch (error) {
    return mobileError('Failed to fetch salary slips', 500);
  }
}

export async function POST(request: NextRequest) {
  const permCheck = await mobileRequireContractorPermission(request, 'salary_slips:create');
  if (!permCheck.authorized) return permCheck.error!;
  try {
    ...
    return mobileSuccess(slip, 'Salary slip created');
  } catch (error) {
    console.error('Mobile create salary slip error:', error);
    return mobileError('Failed to create salary slip', 500);
  }
}
