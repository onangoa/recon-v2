...
export async function POST(
  ...
  const permCheck = await mobileRequirePermission(request, 'payroll:manage');
  ...
  try {
    ...
    if (!walletId) {
      return mobileError('Wallet ID is required', 400);
    }
    ...
    if (period.status !== 'completed') {
      return mobileError('Payroll must be completed before disbursement', 400);
    }

    if (wallet.balance < period.totalNetPay) {
      return mobileError(`Insufficient wallet balance. Balance: ${wallet.balance}, Required: ${period.totalNetPay}`, 400);
    }
    ...
    return mobileSuccess(results, 'Disbursement processed');
  } catch (error: any) {
    console.error('Mobile payroll disbursement error:', error);
    return mobileError(error.message || 'Failed to process disbursement', 500);
  }
}
